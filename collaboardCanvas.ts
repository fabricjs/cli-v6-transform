...
import {
  selectActivatePenOnTouch,
  selectCanvasGrid,
  selectEmbedTransparency,
  selectObjectGradients,
} from "../../../reduxStore/canvas/userSettings/userSettings.reducer";
import {
  selectVotingSession,
  VotingState,
} from "../../../reduxStore/canvas/voting/voting.reducer";
...
export interface BaseCollaboardCanvas {
  ...
  initialize(
    ...
    options: Partial<fabric.CollaboardCanvas>
  ): void;
...
const mixin: Partial<BaseCollaboardCanvas & fabric.Canvas> = {
  ...
  isViewOnlyMode: false, // TODO: #5927 - remove (single source of truth)
  ...
  initialize(
    ...
    this.requestRepositionRelativeElements = debounce(() =>
      document.dispatchEvent(new CustomEvent(repositionRelativeElementsEvent))
    );
  ...
  attachCustomEvents(this: fabric.CollaboardCanvas) {
    ...
    this.on("object:rotating", function ({ target }) {
      hasInnerObjects(target) && target.onRotating();
    });
    this.on("object:rotated", function ({ target }) {
      hasInnerObjects(target) && target.onRotated();
    });

    this.on("object:scaling", function ({ target }) {
      hasInnerObjects(target) && target.onScaling();
    });
    this.on("object:scaled", function ({ target }) {
      hasInnerObjects(target) && target.onScaled();
    });
    this.on("object:moving", function ({ target }) {
      hasInnerObjects(target) && target.onMoving();
    });
    this.on("object:moved", function ({ target }) {
      hasInnerObjects(target) && target.onMoved();
    });

    this.on("before:transform", ({ transform }) => {
      if (!transform) {
        return;
      }
      const { target, action } = transform;

      if (
        ["rotate", "scale", "drag", "move"].includes(action) &&
    ...
    this.on("after:transform", ({ transform }) => {
    ...
    this.on("custom:canvas:set:urls:clickable", function ({ urlsClickable }) {
  ...
  attachDOMEventHandlers(this: fabric.CollaboardCanvas) {
    ...
    Object.entries(this.windowEventHandlers).forEach(
...
fabric.CollaboardCanvas = fabric.util.createClass(fabric.Canvas, mixin);
